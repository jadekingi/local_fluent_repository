...
for sgv_index = 1:1:4
	...
	riser_liq = bin3_data_liq{3,sgv_index,1};
	downcomer_gas = bin3_data_GasHoldUp_Bins{3,sgv_index,2};
	downcomer_liq = bin3_data_liq{3,sgv_index,2};
	C0 = 0.4;
	Dl = 1.91e-9;
	max_index = size(riser_liq);
	for index = 1:1:max_index(1,1)
		v = riser_liq(index,4);
		epsilon = riser_liq(index,6);
		kl = C0*(Dl/v)^(2/3)*(epsilon*v)^(1/4);
		a_intfacial = 6*riser_gas(index,16)*riser_liq(index,5)/riser_gas(index,4);
		riser_kl_kla(index,1) = riser_liq(index,1);
		riser_kl_kla(index,2) = riser_liq(index,2);
		riser_kl_kla(index,3) = kl;
		riser_kl_kla(index,4) = kl*a_intfacial;
	end
	max_index_d = size(downcomer_liq);
	for index = 1:1:max_index_d(1,1)
		v = downcomer_liq(index,4);
		epsilon = downcomer_liq(index,6);
		kl = C0*(Dl/v)^(2/3)*(epsilon*v)^(1/4);
		downcomer_kl(index,1) = downcomer_liq(index,1);
		downcomer_kl(index,2) = downcomer_liq(index,2);
		downcomer_kl(index,3) = kl;
	end
	riser_kl_kla_ave(sgv_index,1) = sgv_index*0.01;
	riser_kl_kla_ave(sgv_index,2) = mean(riser_kl_kla(:,3));%kl and kla average with GSV
	riser_kl_kla_ave(sgv_index,3) = mean(riser_kl_kla(:,4));%kl and kla average with GSV
...
figure
plot(riser_kl_kla_Y_results(:,1,1),riser_kl_kla_Y_results(:,2,1),'o','linestyle','-','linewidth',1,'Color','#151f30','markerfacecolor','#151f30');
hold on
plot(riser_kl_kla_Y_results(1:48,1,2),riser_kl_kla_Y_results(1:48,2,2),'s','linestyle','-','linewidth',1,'Color','#103778','markerfacecolor','#103778');
...
figure
plot(riser_kl_kla_Y_results(:,1,1),riser_kl_kla_Y_results(:,3,1),'o','linestyle','-','linewidth',1,'Color','#151f30','markerfacecolor','#151f30');
hold on
plot(riser_kl_kla_Y_results(1:48,1,2),riser_kl_kla_Y_results(1:48,3,2),'s','linestyle','-','linewidth',1,'Color','#103778','markerfacecolor','#103778');
